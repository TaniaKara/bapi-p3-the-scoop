...
function deleteComment(url, request){
  ...
  if(!id){
    ...
    database.comments[id] = null; // delete comment from db
...
function saveDatabase(){
  let myDB = yaml.dump(database);
  fs.writeFileSync(_dirDB, myDB);
}

function loadDatabase(){
  if(!fs.existsSync(_dirDB)){
    fs.openSync(_dirDB,'w');
  }
  let object = fs.readFileSync(_dirDB, 'utf8');
  let db = yaml.safeLoad(object);
  return db;
}
